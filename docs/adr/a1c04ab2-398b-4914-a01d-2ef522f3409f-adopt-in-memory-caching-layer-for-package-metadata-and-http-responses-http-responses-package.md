...
1. MUST: HTTP responses from package registries MUST be cached using an HTTP cache implementation that respects cache-control headers and provides disk-backed persistence

## Policy Block

- MUST HTTP responses from package registries MUST be cached using an HTTP cache implementation that respects cache-control headers and provides disk-backed persistence
