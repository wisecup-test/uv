...
1. MUST: Tracing spans MUST be structured with consistent field names following OpenTelemetry semantic conventions for HTTP operations (http.method, http.url, http.status_code, http.response_content_length)

## Policy Block

- MUST Tracing spans MUST be structured with consistent field names following OpenTelemetry semantic conventions for HTTP operations (http.method, http.url, http.status_code, http.response_content_length)
