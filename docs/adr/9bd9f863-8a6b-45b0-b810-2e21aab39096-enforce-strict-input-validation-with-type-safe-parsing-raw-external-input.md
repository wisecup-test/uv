...
1. MUST_NOT: Raw external input MUST NOT be used in security-sensitive operations (file paths, SQL queries, shell commands) without validation and sanitization

## Policy Block

- MUST_NOT Raw external input MUST NOT be used in security-sensitive operations (file paths, SQL queries, shell commands) without validation and sanitization
